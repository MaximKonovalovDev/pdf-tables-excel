...
| Pillow | 12.3.0 | HPND (historical permissive notice) | Image format support for OCR rendering |
...
- **Pillow** — HPND-style historical permission notice (the license Pillow
   ...
   Full text: `licenses/Pillow-12.3.0-LICENSE.txt`.
...
- **Ghostscript** — AGPL-3.0. Needed only for the lined-grid ("lattice")
   ...
   or redistributes it. When present, Ghostscript is invoked as a **separate
   operating-system process** via Camelot's lattice path (not linked into the
   product). No Ghostscript code ships inside the product; the AGPL
   obligation therefore stays with the user's separate install. Without
   Ghostscript, lattice mode is unavailable but stream and OCR paths work
   normally.
